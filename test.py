...
from utils import reload_ckpt_bis, reload_ckpt, \
...
def main(args):
    ...
    #args.exp_name = "model_1model_best_299"
    ...
    args.checkpoint = "/data5/lh/brats/runs/no_pretrain/model_1model_best_399.pth.tar"
    ...
    if args.model_type == "vtnet":
        ...
        model_1 = ViT_seg(config, num_classes=args.num_classes,
                          ...
                          win_size=yaml_cfg.get("MODEL").get("SWIN").get("WINDOW_SIZE"), mask_modal = args.modal_list).cuda()
        model_1.load_from(config)
        reload_ckpt(f'{args.checkpoint}', model_1, device)
    else:
        model_1 = Unet_missing(input_shape = [128,128,128], out_channels=3, mdp=0, pre_train = False)
        
        args.checkpoint = "/data5/lh/brats/runs/smu_net_mdp0_part_001_2/model_1model_best_249.pth.tar"
