...
def sliding_window_inference(
    ...
    **kwargs: Any,
    ...
    for slice_g in range(0, total_slices, sw_batch_size):
        ...
        #print(unravel_slice[0][2].start, unravel_slice[0][2].stop)
        zmin = unravel_slice[0][2].start
        ymin = unravel_slice[0][3].start
        xmin = unravel_slice[0][4].start
        
        zmax = zmin + roi_size[0]
        ymax = ymin + roi_size[0]
        xmax = xmin + roi_size[0]
        
        crop_indexes=((zmin, zmax), (ymin, ymax), (xmin, xmax))
        
        window_data = torch.cat([inputs[win_slice] for win_slice in unravel_slice]).to(sw_device)
        seg_prob = predictor(window_data, crop_indexes, *args, **kwargs).to(device)  # batched patch segmentation
